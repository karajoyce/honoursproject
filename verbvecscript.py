...
This is the start of the code that is dedicated to creating vectors of the verbs in the dataset. 
...
data = pd.read_csv('verbs_fixed.csv', encoding='utf-16le', names=['English', 'POS', 'Wordnet', 'Dene'])
...
for index, row in en_data.iterrows():
	new_data["English"].append(row['English'].strip().replace("  ", " ").replace("(pl)", "").replace("(dl)", "").replace('(', '').replace(')', '').replace(',', '').replace("see also ", '').replace('"', '').split(" "))
	new_data["Dene"].append(row["Dene"])
	wn_data["Wordnet"].append(row["Wordnet"].strip().replace("  ", " ").replace(" (", "$").replace("(", "").replace(",", '').split("$"))
		...
		for each in row["Wordnet"]:
			...
			for letter in each:
				...
				if letter != ")" and posi == 0:
					...
					sense += letter
			sense = sense.replace(")", "")
			wn_sample = (stem+"."+pos+"."+"0"+sense).strip()
...
for i in range(len(wn_final)):
	#item = vec english, dene, wn english, similarity
	item = []
...
final.to_csv("verbs_out.csv", encoding='utf-8-sig', header=True)
...
mp.title("Verb Similarities")
mp.xlabel("Cosine Similarity")
mp.ylabel("Frequency")
mp.grid(True)
mp.savefig("verbs.png")
